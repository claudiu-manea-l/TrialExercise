...
public class FlagName {
    ...
    public static void setDrawableToImageView(ImageView img,Context context,String name){
        String finalName = FlagName.getInstance().getFlagFilenameForCountry(name,context);
        if(finalName.equals("unknown"))
            finalName = FlagName.getInstance().getFlagFilenameForNationality(name,context);
        int imageID = context.getResources().getIdentifier(finalName, "drawable",
